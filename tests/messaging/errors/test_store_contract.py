...
# Behavioral contract shared by every IDeadLetterStore implementation. Parametrized via the `dlq_store`
# fixture (conftest.py) over the canonical fake and the SQLAlchemy store, so both must behave
# identically. SQLAlchemy-only concerns (SKIP LOCKED, concurrent claim) stay in sqla/.
...
async def test_p2_columns_metadata_group_id_message_id_round_trip(dlq_store: IDeadLetterStore) -> None:
...
async def test_message_id_none_when_not_provided(dlq_store: IDeadLetterStore) -> None:
