...
__all__ = ['CheckpointStoreContract']


class CheckpointStoreContract:
    """Behavioral contract every ``ICheckpointStore`` implementation must pass.

    Subclass in your backend's test suite and override the ``checkpoint_store`` fixture with your
    adapter over a fresh resource per test.
    """

    @pytest.fixture
    def checkpoint_store(self) -> ICheckpointStore:
        msg = 'override the checkpoint_store fixture with your backend adapter'
        raise NotImplementedError(msg)  # pragma: no cover

    async def test_load_returns_none_when_no_checkpoint(self, checkpoint_store: ICheckpointStore) -> None:
        result = await checkpoint_store.load('nonexistent')
        assert result is None

    async def test_save_and_load_round_trip(self, checkpoint_store: ICheckpointStore) -> None:
    ...
    async def test_upsert_replaces_existing(self, checkpoint_store: ICheckpointStore) -> None:
        t1 = datetime.now(UTC)
    ...
    async def test_separate_projections_have_independent_checkpoints(self, checkpoint_store: ICheckpointStore) -> None:
        now = datetime.now(UTC)
